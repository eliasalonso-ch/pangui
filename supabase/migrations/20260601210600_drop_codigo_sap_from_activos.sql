-- Remove codigo_sap from the activity-log trigger first (it referenced the column),
-- then drop the column. No asset uses it (verified: 0 rows with a value).
...
BEGIN
  ...
  IF NEW.codigo          IS DISTINCT FROM OLD.codigo          THEN v_campos := array_append(v_campos, 'Código'); END IF;
...
ALTER TABLE activos DROP COLUMN IF EXISTS codigo_sap;;
