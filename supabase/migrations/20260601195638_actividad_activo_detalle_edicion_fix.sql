-- Fix: use array_append (the `array || text` form was ambiguous and errored).
...
BEGIN
  ...
  IF NEW.codigo_sap      IS DISTINCT FROM OLD.codigo_sap      THEN v_campos := array_append(v_campos, 'Código SAP'); END IF;
