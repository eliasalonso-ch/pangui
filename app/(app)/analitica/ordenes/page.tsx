  ...
  return (
    <div style={{ padding: "28px 32px 64px", minHeight: "100vh", background: C.bg }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 14, marginTop: 14 }}>
        ...
        <StatCard label="En espera" value={blocked.length} sub={`${fmtPct(blocked.length, openOTs.length)} de las pendientes`} icon={Activity} color={blocked.length ? C.warning : C.success} tone={blocked.length > 0 ? "warn" : "good"} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 14, marginTop: 14 }}>
        ...
        <StatCard label="Sub-OTs periodo" value={periodSubOTs.length} sub={`${periodSubOTs.filter(isOpen).length} pendientes`} icon={GitBranch} color={C.info} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 14, marginTop: 14 }}>
        ...
        <StatCard label="SLA vencido" value={fmtPct(overdue.length, openOTs.length)} sub={`${overdue.length}/${openOTs.length} pendientes fuera de fecha`} icon={AlertTriangle} color={overdue.length ? C.danger : C.success} tone={overdue.length ? "bad" : "good"} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}>
        ...
        <Card fixed>
          <CardHeader title="Prioridad pendiente" />
        ...
        <CardHeader
          ...
          action={(
            <div style={segmentedStyle}>
              <button type="button" onClick={() => setPeopleMode("open")} style={segmentButtonStyle(peopleMode === "open")}>Carga pendiente</button>
