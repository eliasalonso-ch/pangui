...
const BLANK: FormState = {
  titulo: "", n_ot: "", solicitante: "", solicitante_telefono: "", solicitante_email: "", hito: "", presupuesto: "", descripcion: "",
...
// *name* string on the OT (matching mobile), plus an optional phone + email that
...
export default function OTCrearPanel({
  ...
  const save = async () => {
    ...
    try {
      // Persist the requester's contact info to the catalog (best-effort) so it
      // can be reused; the OT keeps its own snapshot below.
      if (form.solicitante.trim()) {
        try {
          await upsertSolicitante(wsId, form.solicitante, form.solicitante_telefono, form.solicitante_email);
        } catch { /* non-fatal — the OT snapshot is the source of truth for this OT */ }
      }
