...
const ESTADO_LABEL = { completado: "Completada", cancelado: "Cancelada" };
const ESTADO_COLOR = {
...
function vencimiento(fecha) {
  ...
  if (diff < 0) return { label: `venció hace ${Math.abs(diff)}d`, urgent: false };
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
        ...
        <div className={styles.pageHeader}>
          <div className={styles.pageHeaderLeft}>
            ...
            <h1 className={styles.pageHeaderTitle}>Completadas</h1>
          </div>
        ...
        <div className={styles.sectionTabs}>
          <button className={styles.sectionTab} type="button" onClick={() => router.push("/ordenes")}>
            Pendientes
          </button>
          <button className={`${styles.sectionTab} ${styles.sectionTabActive}`} type="button" onClick={() => router.push("/ordenes/completadas")}>
            Completadas
          </button>
        </div>
        ...
        <div className={styles.otList}>
          {loading ? (
            ...
            <div className={styles.emptyState}>Sin órdenes completadas</div>
          ) : lista.map(o => {
              ...
              <button key={o.id} className={styles.otCard} onClick={() => router.push(`/ordenes/${o.id}`)}>
                <div className={styles.otCardHeader}>
                  ...
                  <span className={styles.otTimeAgo}>
                    {new Date(o.created_at).toLocaleDateString("es-CL", { day: "numeric", month: "short" })}
                  </span>
                </div>
                ...
                <div className={styles.otCardMeta}>
                  ...
                  {venc && <span className={styles.otMeta}><Clock size={11} />{venc.label}</span>}
...
function CompletadasCalendarOverlay({ onClose, calendarDate, setCalendarDate, ordenes, ESTADO_LABEL, ESTADO_COLOR }) {
  const [viewMonth, setViewMonth] = useState(() => { const d = calendarDate || new Date(); return { year: d.getFullYear(), month: d.getMonth() }; });
  ...
  const countByDay = {};
  ordenes.forEach(o => { const d = new Date(o.created_at); if (d.getFullYear() === viewMonth.year && d.getMonth() === viewMonth.month) countByDay[d.getDate()] = (countByDay[d.getDate()] || 0) + 1; });
    ...
    <div className={styles.overlayFull}>
      ...
      <div className={styles.calendarWrap}>
        <div className={styles.calendarNav}>
          <button className={styles.calNavBtn} onClick={() => { const d = new Date(viewMonth.year, viewMonth.month - 1, 1); setViewMonth({ year: d.getFullYear(), month: d.getMonth() }); }} type="button"><ChevronLeft size={18} /></button>
          <span className={styles.calMonthLabel}>{monthLabel}</span>
          <button className={styles.calNavBtn} onClick={() => { const d = new Date(viewMonth.year, viewMonth.month + 1, 1); setViewMonth({ year: d.getFullYear(), month: d.getMonth() }); }} type="button"><ChevronRight size={18} /></button>
        </div>
        ...
        <div className={styles.calGrid}>
          {cells.map((day, i) => {
            ...
            return (<button key={day} type="button" className={`${styles.calDay} ${sameDay(thisDate, today) ? styles.calDayToday : ""} ${sameDay(thisDate, calendarDate) ? styles.calDaySelected : ""}`} onClick={() => { const d = new Date(viewMonth.year, viewMonth.month, day); setCalendarDate(sameDay(d, calendarDate) ? null : d); }}>{day}{(countByDay[day] || 0) > 0 && <span className={styles.calDayDot} />}</button>);
          })}
        </div>
      </div>
      <div className={styles.calResults}>
        {calendarDate ? (<><p className={styles.calResultsLabel}>{ordenesDia.length > 0 ? `${ordenesDia.length} orden${ordenesDia.length !== 1 ? "es" : ""} — ${calendarDate.toLocaleDateString("es-CL", { day: "numeric", month: "long" })}` : `Sin órdenes el ${calendarDate.toLocaleDateString("es-CL", { day: "numeric", month: "long" })}`}</p><div className={styles.calResultsList}>{ordenesDia.map(o => { const est = ESTADO_COLOR[o.estado] ?? { bg: "#F3F4F6", text: "#6B7280" }; return (<div key={o.id} className={styles.calResultCard}><span className={styles.otStatusBadge} style={{ background: est.bg, color: est.text, fontSize: 11 }}>{ESTADO_LABEL[o.estado]}</span><p className={styles.calResultTitle}>{o.titulo || "Sin título"}</p></div>); })}</div></>) : <p className={styles.calHint}>Selecciona un día para ver las órdenes</p>}
      </div>
...
function CompletadasFilterOverlay({ onClose, filtros, setFiltros, sortBy, setSortBy, usuarios, ubicaciones, activos, SORT_OPTS, PRIORIDAD_COLOR, PRIORIDAD_LABEL, TIPO_TRABAJO_LABEL }) {
  ...
  const activeCount = [filtros.asignado_a.length > 0, filtros.prioridad.length > 0, filtros.ubicacion_id.length > 0, filtros.activo_id.length > 0, filtros.tipo_trabajo.length > 0, !!(filtros.created_desde || filtros.created_hasta)].filter(Boolean).length;
  return (
    <div className={styles.overlayFull}>
      <div className={styles.overlayHeader}>
        <button className={styles.overlayBack} onClick={onClose} type="button"><X size={20} /></button>
      ...
      <div className={styles.filterBody}>
        <div className={styles.filterSection}><p className={styles.filterSectionLabel}>Ordenar por</p><div className={styles.filterChipRow}>{SORT_OPTS.map(o => (<button key={o.value} type="button" className={`${styles.filterChip} ${sortBy === o.value ? styles.filterChipActive : ""}`} onClick={() => setSortBy(o.value)}>{o.label}</button>))}</div></div>
        <div className={styles.filterSection}><p className={styles.filterSectionLabel}>Prioridad</p><div className={styles.filterChipRow}>{["urgente","alta","media","baja","ninguna"].map(v => (<button key={v} type="button" className={`${styles.filterChip} ${filtros.prioridad.includes(v) ? styles.filterChipActive : ""}`} onClick={() => toggle("prioridad", v)} style={filtros.prioridad.includes(v) && PRIORIDAD_COLOR[v] ? { borderColor: PRIORIDAD_COLOR[v], color: PRIORIDAD_COLOR[v], background: PRIORIDAD_COLOR[v] + "18" } : {}}>{PRIORIDAD_LABEL[v]}</button>))}</div></div>
