        ...
        <div style={{ border: "1px solid var(--border)", borderRadius: "var(--r-lg)", overflow: "hidden", background: "var(--surface-1)" }}>
          {loading && !result ? (
            ...
            <div style={{ minHeight: 280, padding: 32, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
              ...
              <p style={{ margin: 0, color: "var(--fg-1)", fontSize: 14, fontWeight: 400 }}>Aún no hay documentos</p>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 760, color: "var(--fg-1)", fontSize: 14 }}>
                <thead>
                  <tr style={{ background: "var(--surface-2)" }}>
                    {['Fecha', 'Tipo', 'Número', 'Monto', 'Estado'].map((heading, index) => (
                      <th key={`${heading}-${index}`} style={{ height: 46, padding: "0 16px", textAlign: "left", fontWeight: 400, borderBottom: "1px solid var(--border)", whiteSpace: "nowrap" }}>{heading}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {invoices.map((invoice, index) => (
                    <tr key={invoice.id}>
                      ...
                      <td style={{ ...cell(index === invoices.length - 1), fontVariantNumeric: "tabular-nums", fontWeight: 400 }}>{money(invoice.amount, invoice.currency)}</td>
