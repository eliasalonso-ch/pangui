        ...
        {props.showPlanSummary && (
          <div style={panelStyle}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 660, fontSize: 14, color: "var(--fg-1)" }}>
                <thead>
            ...
            <p style={{ margin: 0, padding: "0 14px 12px", fontSize: 14, color: "var(--fg-3)", lineHeight: 1.5 }}>
              {props.canceledAt
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
          ...
          <SideCard title="Datos de facturación" icon={ReceiptText} onEdit={() => { setDraft(profile); setError(null); setEditingAddress(true); }}>
            {loading ? <div style={{ flex: 1, display: "grid", placeItems: "center", padding: 16, color: "var(--fg-4)" }}><Loader2 size={16} className="animate-spin" /></div> : profileComplete ? (
              ...
              <button type="button" onClick={() => { setDraft(profile); setError(null); setEditingAddress(true); }} style={{ flex: 1, margin: 14, padding: 12, border: "1px dashed var(--border-strong)", borderRadius: "var(--r-md)", background: "transparent", color: "var(--brand)", cursor: "pointer", textAlign: "left", fontSize: 14, fontFamily: "inherit" }}>
          ...
          <SideCard title="Método de pago" icon={CreditCard}>
            {hasCard ? (
              ...
              <div style={{ padding: 16, display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                <p style={{ margin: 0, fontSize: 14, lineHeight: 1.5, color: "var(--warning)", maxWidth: 460 }}>
      ...
      {editingEmail ? (
        <ModalShell title="Email de cobros" width={410} onClose={() => setEditingEmail(false)}>
          <div style={{ padding: 18, display: "grid", gap: 16 }}>
            <p style={{ margin: 0, color: "var(--fg-3)", fontSize: 14 }}>Email actual: {profile.billing_email || props.billingEmail || "Sin configurar"}</p>
      ...
      {editingAddress ? (
        ...
        <ModalShell title="Datos de facturación" width={620} onClose={() => setEditingAddress(false)}>
          ...
          <div style={{ padding: 18, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: 14 }}>
            <p style={{ margin: "0 0 2px", gridColumn: "1 / -1", color: "var(--fg-3)", fontSize: 14, lineHeight: 1.45 }}>
            ...
            <label style={{ display: "grid", gap: 6, color: "var(--fg-2)", fontSize: 14, fontWeight: 400 }}>
              Región
              <select
                value={draft.region ?? ""}
                onChange={event => {
...
function SideCard({ title, icon: Icon, onEdit, children }: { title: string; icon: LucideIcon; onEdit?: () => void; children: React.ReactNode }) {
  return (
    <section style={{ ...panelStyle, height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ height: 40, padding: "0 14px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid var(--border)", color: "var(--fg-3)", fontSize: 14, fontWeight: 400 }}>
...
function Header({ children }: { children: React.ReactNode }) { return <th style={{ height: 46, padding: "0 16px", textAlign: "left", borderBottom: "1px solid var(--border)", fontWeight: 400 }}>{children}</th>; }
...
function Field({ label, value, onChange, wide, placeholder }: { label: string; value: string | null; onChange: (value: string) => void; wide?: boolean; placeholder?: string }) { return <label style={{ display: "grid", gap: 6, gridColumn: wide ? "1 / -1" : undefined, color: "var(--fg-2)", fontSize: 14, fontWeight: 400 }}>{label}<input value={value ?? ""} placeholder={placeholder} onChange={event => onChange(event.target.value)} style={inputStyle} /></label>; }
function statusStyle(status: string): React.CSSProperties { const active = status === "active" || status === "trialing"; return { width: "fit-content", padding: "3px 9px", border: "1px solid var(--border)", borderRadius: 999, color: active ? "var(--success)" : status === "past_due" ? "var(--warning)" : "var(--fg-3)", fontSize: 14, fontWeight: 400 }; }
...
const sideRow: React.CSSProperties = { flex: 1, padding: 14, display: "flex", alignItems: "center", gap: 9, color: "var(--fg-2)", fontSize: 14, lineHeight: 1.45 };
const subtext: React.CSSProperties = { color: "var(--fg-4)", fontSize: 14, fontWeight: 400 };
const smallButton: React.CSSProperties = { minHeight: 30, padding: "0 10px", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, border: "1px solid var(--border)", borderRadius: "var(--r-md)", background: "var(--surface-1)", color: "var(--fg-1)", fontSize: 14, fontWeight: 400, cursor: "pointer" };
