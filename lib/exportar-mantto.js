...
export async function exportarPDF(ordenes, filtros = {}) {
  ...
  function drawHeader(subtitulo) {
    ...
    doc.text("Pangui", 10, 12);
    const panguiW = doc.getTextWidth("Pangui");
    doc.setTextColor(...blanco);
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text(subtitulo, 10 + panguiW + 6, 12);
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(...gris);
    doc.text(`Pangui · Reporte de mantenimiento · ${fechaHoy}`, 10, h - 5);
    doc.text(`Página ${i} de ${pages}`, w - 10, h - 5, { align: "right" });
  }

  doc.save(`pangui_reporte_${desde}_${hasta}.pdf`);
}
...
export async function exportarExcel(ordenes, filtros = {}) {
  ...
  const datosOrdenes = ordenes.map((o) => {
    ...
    return {
      ...
      N_Referencia: o.numero_meconecta || "—",
  ...
  XLSX.writeFile(wb, `pangui_reporte_${desde}_${hasta}.xlsx`);
}
